...
int main() {
    ...
    CROW_ROUTE(app, "/")([](){
        crow::response res("풋살 동호회 서버가 작동중입니다!");
    ...
    ([](const crow::request& req){
        ...
        if (!body) {
            return crow::response(400, "잘못된 요청입니다.");
        }
...
// MVP 등록 (관리자 전용 - 비밀번호 확인)
CROW_ROUTE(app, "/mvp/add").methods("POST"_method)
([](const crow::request& req){
    auto body = crow::json::load(req.body);
    if (!body) {
        return crow::response(400, "잘못된 요청입니다.");
...
([](){
    ...
    int count = 0;
    int idx = 0;
    for (int i = links.size() - 1; i >= 0 && count < 5; i--) {
...
// 경기 영상 등록 (관리자 전용)
CROW_ROUTE(app, "/videos/add").methods("POST"_method)
([](const crow::request& req){
    auto body = crow::json::load(req.body);
    if (!body) {
        return crow::response(400, "잘못된 요청입니다.");
    ...
    // 포인트 수정 (관리자 전용)
CROW_ROUTE(app, "/member/points").methods("POST"_method)
([](const crow::request& req){
    auto body = crow::json::load(req.body);
    if (!body) {
        return crow::response(400, "잘못된 요청입니다.");
    ...
    // 회원 추가 (관리자 전용)
CROW_ROUTE(app, "/member/add").methods("POST"_method)
([](const crow::request& req){
    auto body = crow::json::load(req.body);
    if (!body) return crow::response(400, "잘못된 요청입니다.");
...
// 회원 정보 수정 (생년월일/레벨, 관리자 전용)
CROW_ROUTE(app, "/member/update").methods("POST"_method)
([](const crow::request& req){
    auto body = crow::json::load(req.body);
    if (!body) return crow::response(400, "잘못된 요청입니다.");
    ...
    if (body.has("birthdate")) {
        members[idx].birthdate = body["birthdate"].s();
    }
    if (body.has("skillLevel")) {
        members[idx].skillLevel = body["skillLevel"].i();
    }
...
// 회원 삭제 (관리자 전용)
CROW_ROUTE(app, "/member/delete").methods("POST"_method)
([](const crow::request& req){
    auto body = crow::json::load(req.body);
    if (!body) return crow::response(400, "잘못된 요청입니다.");
    ...
    // 팀 자동 배정 (관리자 전용)
CROW_ROUTE(app, "/team/assign").methods("POST"_method)
([](const crow::request& req){
    auto body = crow::json::load(req.body);
    if (!body) return crow::response(400, "잘못된 요청입니다.");
    ...
    for (int i = 0; i < teamA.size(); i++) {
        result["teamA"][i] = teamA[i].name;
    }
    result["teamB"] = crow::json::wvalue::list();
    for (int i = 0; i < teamB.size(); i++) {
        result["teamB"][i] = teamB[i].name;
    }
